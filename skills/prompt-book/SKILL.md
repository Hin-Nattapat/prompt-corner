...
4. **Write map's §2 phase map + §3 contracts.** §2: per phase — what it delivers, which repo, the contract it sets for later phases, what it must NOT touch, prerequisites, `blocked` label where relevant. A phase you cannot yet state a deliverable and a contract for is not a phase: it goes under §2's `### ยังไม่ชัด` as one line — the question it hangs on and which phase's landing will sharpen it — and stays out of the frontmatter `phases` list. Don't pre-slice fog into phase-sized pieces; one line there may become two phases or none once the phase before it lands. §3: per contract — which phase sets it, which phase consumes it, its concrete form, ✅/❓.
5. **Write map's §4 premises** — six columns: id (`P<n>`) | premise | type (`internal`/`external-doc`/`external-api`) | status (✅ / ❌ เท็จ / 🔒 / 🔓 waived / ❓ unverified — treat like 🔒, don't plan past it) | evidence/anchor | who can confirm. Source every row from the §0 sweep: open each doc you listed there and read it in full — a gotchas file especially, they are premises end to end — and lift every claim this work depends on into its own row with that doc's path and a grep-able text anchor. Every `internal` row also needs a real `file:line`. Never write a premise from memory; if it isn't backed by something you just read or grepped, it doesn't get a row.

   **What earns a row — a grep must not be able to settle it.** `external-doc` and `external-api` rows always qualify: they are somebody else's document or somebody else's payload, and reading our own code proves only what we read, never what they send. An `internal` row qualifies only while it is **contested** — two readings exist, a previous answer turned out false, or a comment says one thing and the code another. A plain internal fact a `git grep` settles on demand belongs in the phase's own plan, not here: it costs a row, a verify line and a slice of every session the map is injected into, and it was never going to be the thing that surprised a later phase. Measured across two finished programs, every premise that stayed blocking to the end was `external-api`, and a table that was 31/34 `internal` produced no false premise at all. The id assigned here is the same id the verify script uses for that row's `chk`/`ask` line — nothing else ties a script's `FAIL P3` back to a row.
6. **Batch every 🔒 and ❓ question into one file and stop.** Write `<programs-dir>/<program>.questions.md`, one block per row, then tell the user the path and wait — don't trickle questions in as you hit them, and don't ask in the chat, which a compact erases. Right timing is now, before phase 1, not mid-phase when blocked.

   ```
   ## P4 — LM sends `merchant_id` (snake) in the order webhook
   evidence: partners/lineman/gotchas.md:88 · who: LM team
   options: (a) snake, keep the mapper · (b) camel, phase 1 re-maps · (c) unknown, waive with assumption (a)
   answer:
   ```

   The user answers in the file or in the chat; either way the answer lands in the map's §4 row (✅ with the new evidence, 🔓 waived with its assumption, or 🔒 still open) before step 7, and the block is deleted. A block whose `answer:` is still blank when the file is read back is a 🔒 — the verify script's `ask` line keeps printing it every session until it is not.
