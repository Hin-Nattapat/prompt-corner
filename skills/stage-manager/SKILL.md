...
3. Per phase, in order:
   ...
   - Implement the plan. An implementation agent's report ends with **`ambiguities:`** — every place the spec or plan left a choice open, what it chose, `file:line`. Before `notes-call` runs, the controller writes each one into the spec or plan (as a decision, not prose) or turns it into an O bullet for the STOP; none is left in the report alone. Measured, that paragraph was the highest-value one in each implementation report, and three of its entries became spec amendments the fix round then depended on.
   ...
   - **STOP.** Report and wait for the user. `notes-call`'s `รอเจ้าของเคาะ (O)` bullets are decided here — each one is a plan sentence the user keeps or changes, not a `curtain-hold` case; an `O (ทำไปก่อน)` is a spec edit already made that the user confirms or reverts. The STOP report carries one line the review report cannot: `Tokens (phase): impl <i1>+<i2>+… · review <notes-call's sum> = <total>k` — the phase's whole subagent cost, not only the review's.
   - On "push and pr" — run the git tail, and delete this feature's `<program>.handoff.md` and `<program>.handoff.auto.md` if they exist; the position they recorded is now the PR. Default: branch off the base branch, group into logical commits, rebase, push, open the PR, hand back the URL — never merge. `house.md`'s `## Git tail` section wins where it says otherwise, including its own trailing steps. A phase touching more than one repo runs that tail once per repo and lands one PR per repo — never treat one repo's merge as the phase merging.
   - Move to the next phase only after the user has acted on the STOP above — and first read §2's `### ยังไม่ชัด`: a line there that this phase's landing has sharpened becomes a phase now (via `prompt-book`, which updates `phases:` and `last-touched`), before the next phase is picked.
